"This was made by Reddit user u/SamLovesNotion. Also with the help of -
"https://tdaly.co.uk/projects/vim-statusline-generator/ for learning the syntax. Sorry for English & grammar, this post was made in hurry.
...
" This statusline looks exactly like Vim Airline (even more customizable &
"powerful) & loads faster than Vim airline. Only take few ms to load.

" STARTUP TIME - With Vim Airline - ~250ms. With this statusline - ~100ms.
"Without any statusline - ~98ms.

" Add all of this at the end of your vimrc OR Create separate file like
"'statusline.vim' & 'colorsgroup.vim' & source those files in your main vimrc.
" e.g. source "~/.config/vim/statusline.vim"



" Color highlighting groups
" Add this AFTER `colorscheme` option in your vimrc. Otherwise your
"colorscheme will clear this highlightings. OR use ColorScheme autocommand.
"VERY IMPORTANT.
...
" Disable showmode - i.e. Don't show mode like --INSERT-- in
"current statusline.
set noshowmode

" Enable GUI colors for terminals (Some terminals may not support
"this, so you'll have to *manually* set color pallet for tui colors.
"Lie tuibg=255, tuifg=120, etc.).
set termguicolors
...
" %{&fenc!='utf-8'?\"\ \":''}   = If file encoding is NOT!= 'utf-8'
"? THEN output a "Space" else : no character 

" Define active statusline

function! ActivateStatusline()
	call GetFileType()
	setlocal statusline=%#StslinePriColorBG#\
	%{StslineMode()}%#StslineSecColorBG#%{get(b:,'coc_git_status',b:GitBranch)}%{get(b:,'coc_git_blame','')}%#StslineBackColorFGPriColorBG#%#StslinePriColorFG#\
	%{&readonly?\"\ \":\"\"}%F\ %#StslineModColorFG#%{&modified?\"\
	\":\"\"}%=%#StslinePriColorFG#\ %{b:FiletypeIcon}%{&filetype}\
	%#StslineSecColorFG#%#StslineSecColorBG#%{&fenc!='utf-8'?\"\
	\":''}%{&fenc!='utf-8'?&fenc:''}%{&fenc!='utf-8'?\"\
	\":''}%#StslinePriColorFGSecColorBG#%#StslinePriColorBG#\ %p\%%\
	%#StslinePriColorBGBold#%l%#StslinePriColorBG#/%L\ :%c\ 
endfunction
...
function! DeactivateStatusline()
	...
	if !exists("b:GitBranch") || b:GitBranch == ''
		setlocal statusline=%#StslineSecColorBG#\ INACTIVE\
		%#StslineSecColorBG#%{get(b:,'coc_git_statusline',b:GitBranch)}%{get(b:,'coc_git_blame','')}%#StslineBackColorFGSecColorBG#%#StslineBackColorBG#\
		%{&readonly?\"\ \":\"\"}%F\ %#StslineModColorFG#%{&modified?\"\
		\":\"\"}%=%#StslineBackColorBG#\ %{b:FiletypeIcon}%{&filetype}\
		%#StslineSecColorFGBackColorBG#%#StslineSecColorBG#\ %p\%%\ %l/%L\
		:%c\ 

		else
		setlocal statusline=%#StslineSecColorBG#%{get(b:,'coc_git_statusline',b:GitBranch)}%{get(b:,'coc_git_blame','')}%#StslineBackColorFGSecColorBG#%#StslineBackColorBG#\
		%{&readonly?\"\ \":\"\"}%F\ %#StslineModColorFG#%{&modified?\"\
		\":\"\"}%=%#StslineBackColorBG#\ %{b:FiletypeIcon}%{&filetype}\
		%#StslineSecColorFGBackColorBG#%#StslineSecColorBG#\ %p\%%\ %l/%L\
		:%c\ 
